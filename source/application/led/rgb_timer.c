...
void timer_clear(void) {
    reset_point = (uint32_t)Time_GetTick();
#if CH_CFG_ST_RESOLUTION < 32
...
uint32_t timer_read32(void) {
    uint32_t systime = (uint32_t)Time_GetTick();
...
//     return (uint32_t)TIME_TICK2MS(systime - reset_point + overflow);
// #else
//     return (uint32_t)TIME_TICK2MS(systime - reset_point);
// #endif
 return (uint32_t)TIME_TICK2MS(systime);
}

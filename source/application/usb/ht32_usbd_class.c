...
#define OTA_MASK_H (0xC2)
#define OTA_MASK_L (0xA0)

/* Private types -------------------------------------------------------------------------------------------*/
uint8_t buffer_dpi_idx_xy; // default is Y config
uint8_t buffer_dpi_mode;
uint8_t buffer_dpi_idx_x;
...
__ALIGN4 static uc8 USB_HID_KeyboardReportDesc[] = {
    ...
    // 0x85, HID_REPORT_BOOTLOADER_SETTINGS_ID,             //   REPORT_ID (n)
    // 0x09, 0x02,                       //   USAGE (Vendor Usage 2)
    // 0x15, 0x00,                       //   LOGICAL_MINIMUM (0)
    // 0x26, 0xff, 0x00,                 //   LOGICAL_MAXIMUM (255)
    // 0x75, 0x08,                       //   REPORT_SIZE (8)
    // 0x96, DESC_H2B(HID_REPORT_BOOTLOADER_SETTINGS_LEN),  //   REPORT_COUNT (n)
    // 0xB1, 0x02,                       //   INPUT/OUTPUT/FEATURE (Data, Var, Abs)
...
static void USBDClass_GetReport(USBDCore_Device_TypeDef *pDev)
{
    ...
    case HID_RPOT_TYPE_03_FEATURE:
    {
        ...
        {
            __DBG_USBPrintf("USBDClass_GetReport HID_REPORT_MOUSE_SETTINGS_ID\r\n");
            rgb_matrix_config.dpi_level_xy = buffer_dpi_idx_xy;
            rgb_matrix_config.dpi_level_x = buffer_dpi_idx_x;
            rgb_matrix_config.dpi_mode = buffer_dpi_mode;

            memcpy((void *)&gFeatureReport1Buffer, (void *)&rgb_matrix_config, sizeof(rgb_config_t));
        ...
        else if (id == HID_REPORT_BOOTLOADER_SETTINGS_ID) // use this in the future
        {
            __DBG_USBPrintf("USBDClass_GetReport HID_REPORT_BOOTLOADER_SETTINGS_ID\r\n");
            pDev->Transfer.pData = (uc8 *)&(gFeatureReport2Buffer);
            pDev->Transfer.sByteLength = HID_REPORT_BOOTLOADER_SETTINGS_LEN + 1;
            pDev->Transfer.Action = USB_ACTION_DATAIN;
        }
        break;
    }
...
static void USBDClass_SetReport(USBDCore_Device_TypeDef *pDev)
{
    ...
    case HID_RPOT_TYPE_03_FEATURE:
    {
        if (id == HID_REPORT_MOUSE_SETTINGS_ID)
        {
            printf("USBDClass_SetReport HID_REPORT_MOUSE_SETTINGS_ID\n");
            pDev->Transfer.pData = (uc8 *)&(gFeatureReport1Buffer);
            pDev->Transfer.sByteLength = len;
        ...
        {
            printf("USBDClass_SetReport HID_REPORT_MACROS_SETTINGS_ID\n");
            pDev->Transfer.pData = (uc8 *)&(gFeatureReport2Buffer);
            pDev->Transfer.sByteLength = len;
            pDev->Transfer.Action = USB_ACTION_DATAOUT;
            pDev->Transfer.CallBack_OUT.func = USBDClass_SetReportCallBack0;
        ...
        else if (id == HID_REPORT_BOOTLOADER_SETTINGS_ID)
        {
            printf("USBDClass_SetReport HID_REPORT_BOOTLOADER_SETTINGS_ID\n");
            pDev->Transfer.pData = (uc8 *)&(gFeatureReport2Buffer);
            pDev->Transfer.sByteLength = len;
            pDev->Transfer.Action = USB_ACTION_DATAOUT;
            pDev->Transfer.CallBack_OUT.func = USBDClass_SetReportCallBack0;
...
static void USBDClass_SetReportCallBack0(u32 uPara)
{
    ...
    case HID_REPORT_MOUSE_SETTINGS_ID:
    {
        ...
        printf("rgb_matrix_config buffer_ota %d\r\n", rgb_matrix_config.buffer_ota[0]);
        printf("rgb_matrix_config buffer_ota %d\r\n", rgb_matrix_config.buffer_ota[1]);

        printf("rgb_matrix_config dpi_mode %d\r\n", rgb_matrix_config.dpi_mode);
        printf("rgb_matrix_config dpi_level_xy %d\r\n", rgb_matrix_config.dpi_level_xy);
        printf("rgb_matrix_config dpi_level_x %d\r\n", rgb_matrix_config.dpi_level_x);


        buffer_dpi_mode = rgb_matrix_config.dpi_mode;

        if (rgb_matrix_config.dpi_level_xy > 7)
            rgb_matrix_config.dpi_level_xy = 7;

        if (rgb_matrix_config.dpi_level_x > 7)
            rgb_matrix_config.dpi_level_x = 7;

        if (rgb_matrix_config.dpi_mode == 0)
            buffer_dpi_idx_xy = rgb_matrix_config.dpi_level_xy;
        else
        {
            buffer_dpi_idx_xy = rgb_matrix_config.dpi_level_xy;
            buffer_dpi_idx_x = rgb_matrix_config.dpi_level_x;
        }
        if ((rgb_matrix_config.buffer_ota[0] == OTA_MASK_L) && (rgb_matrix_config.buffer_ota[1] == OTA_MASK_H))
        {
            printf("OTA START\r\n");
        }
        // for (size_t i = 0; i < HID_REPORT_MOUSE_SETTINGS_LEN; i++)
        // {
        //   printf("%x\r\n", gFeatureReport1Buffer[i]);
        // }
    ...
    case HID_REPORT_MACROS_SETTINGS_ID:
        printf("HID_REPORT_MACROS_SETTINGS_ID\r\n");
        break;
    case HID_REPORT_BOOTLOADER_SETTINGS_ID:
        printf("HID_REPORT_BOOTLOADER_SETTINGS_ID\r\n");
        break;
    default:
        break;
    }
